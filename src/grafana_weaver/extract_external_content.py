...
def load_existing_asset_hashes(assets_dir):
    ...
    This creates a mapping of {filename: hash} for all files in the assets
    directory, allowing us to detect if content has changed before overwriting.

    Args:
        assets_dir: Path to the assets directory

    Returns:
        Dictionary mapping filenames to their content hashes
    """
    asset_hashes = {}
...
def process_external_value(
    ...
    Args:
        ...
        asset_hashes: Dictionary mapping filenames to original content hashes (immutable during run)
        written_this_run: Set of filenames that have been written during this run
        written_hashes: Dictionary mapping filenames to hashes of content written during this run
    ...
    Side effects:
        - Writes asset files to assets_dir (only if content changed)
        - Appends modification records to modifications list
        - Saves conflicting content to .conflict files for review
    ...
    for external_line, content in segments:
        ...
        if filename in written_this_run:
            # Check if the content is the same as what was already written
            if written_hashes.get(filename) == new_hash:
                # Same content, this is fine - just skip writing
                print(f"  Skipping {filename} (same content as first panel)")
                should_write = False
            else:
                # CONFLICT: Different content trying to write to the same file
                # Find the next available conflict file number
                conflict_num = 1
                assets_dir.mkdir(exist_ok=True, parents=True)
                while (assets_dir / f"{filename}.conflict{conflict_num}").exists():
                    conflict_num += 1
            ...
            # Check if we need to write the file based on existing content
            should_write = True
            if filename in asset_hashes:
                if asset_hashes[filename] == new_hash:
                    # Content hasn't changed from disk, skip writing
                    should_write = False
                    print(f"  Skipping {filename} (no changes)")
                else:
                    # Content changed from what's on disk
                    print(f"  Updating {filename} (content changed)")
            else:
                # New file
                print(f"  Creating {filename}")

            # Write asset file only if needed
            if should_write:
                assets_dir.mkdir(exist_ok=True, parents=True)
...
def extract_external_content(
    ...
    Args:
        ...
        asset_hashes: Dictionary mapping filenames to original content hashes
        written_this_run: Set of filenames written during this run
        written_hashes: Dictionary mapping filenames to hashes of content written during this run
    ...
    Side effects:
        ...
        - Writes asset files to assets_dir (only if content changed)
        - Appends modification records to modifications list
        - Saves conflict files when multiple panels target same file with different content
...
def process_json_file(json_file_path, base_dir=None, output_dir=None):
    ...
    # Load existing asset hashes to prevent unnecessary overwrites
    print("\nLoading existing assets...")
    asset_hashes = load_existing_asset_hashes(assets_dir)
    if asset_hashes:
        print(f"Found {len(asset_hashes)} existing asset(s)")
    else:
